...
- **자유 형식 원고는 `표준 서식으로 변환` 버튼**으로 맞춘다(본문 라벨 옆). 원고를 그대로 붙여넣고 누르면 `/api/admin/format-update`가 Claude로 위 골격에 맞춰 재구성한다(프롬프트·후처리는 `lib/updateFormat.ts`, 관리자 인증 필수, `ANTHROPIC_API_KEY` 필요). **원고에 없는 내용은 만들지 않으므로 변환 뒤 반드시 미리보기로 검수**한다.
